...
class GammaShieldModule(moduleItem.ModuleItem):
    ...
    def __init__(self, name : str, aliases : List[str], effect : int = 0, value : int = 0,
        ...
        """
        super(GammaShieldModule, self).__init__(name, aliases, value=value, wiki=wiki, manufacturer=manufacturer, icon=icon, emoji=emoji, techLevel=techLevel, builtIn=builtIn)
    ...
    def toDict(self, **kwargs) -> dict:
        ...
        :rtype: dict
        """
        itemDict = super(GammaShieldModule, self).toDict(**kwargs)
        if not self.builtIn:
    ...
    def fromDict(cls, moduleDict : dict, **kwargs):
        ...
        return GammaShieldModule(moduleDict["name"], moduleDict["aliases"] if "aliases" in moduleDict else [], effect=moduleDict["effect"] if "effect" in moduleDict else 1,
