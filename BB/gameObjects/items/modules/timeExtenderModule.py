...
class TimeExtenderModule(moduleItem.ModuleItem):
    ...
    def __init__(self, name : str, aliases : List[str], effect : float = 1, duration : float = 0, value : int = 0,
        ...
        """
        super(TimeExtenderModule, self).__init__(name, aliases, value=value, wiki=wiki, manufacturer=manufacturer, icon=icon, emoji=emoji, techLevel=techLevel, builtIn=builtIn)
    ...
    def fromDict(cls, moduleDict : dict, **kwargs):
        ...
        return TimeExtenderModule(moduleDict["name"], moduleDict["aliases"] if "aliases" in moduleDict else [], effect=moduleDict["effect"] if "effect" in moduleDict else 1,
