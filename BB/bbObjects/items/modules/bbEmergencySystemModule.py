...
class bbEmergencySystemModule(bbModule.bbModule):
    ...
    def __init__(self, name : str, aliases : List[str], duration=0, value=0, wiki="", manufacturer="", icon="", emoji=bbUtil.EMPTY_DUMBEMOJI, techLevel=-1, builtIn=False):
        """
        ...
        """
        super(bbEmergencySystemModule, self).__init__(name, aliases, value=value, wiki=wiki, manufacturer=manufacturer, icon=icon, emoji=emoji, techLevel=techLevel, builtIn=builtIn)
...
def fromDict(moduleDict : dict) -> bbEmergencySystemModule:
