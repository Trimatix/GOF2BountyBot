...
async def admin_cmd_make_role_menu(message : discord.Message, args : str, isDM : bool):
    ...
    if timeoutExists:
        timeoutDelta = lib.timeUtil.timeDeltaFromDict(bbConfig.timeouts.roleMenuExpiry if timeoutDict == {} else timeoutDict)
...
async def admin_cmd_showmeHD(message : discord.Message, args : str, isDM : bool):
    ...
    skinPaths = {0:CWD + os.sep + bbConfig.paths.rendererTempFolder + os.sep + str(message.id) + "_0.jpg"}
    disabledLayers = []
        ...
        for regionNum in pickedLayers:
            ...
            try:
                ...
                if (not nextLayer.filename.lower().endswith(".jpg")) or not (nextLayer.width == 2048 and nextLayer.height == 2048):
                    ...
                    return
                try:
                    await nextLayer.save(CWD + os.sep + bbConfig.paths.rendererTempFolder + os.sep + str(message.id) + "_" + str(regionNum) + ".jpg")
                except (discord.HTTPException, discord.NotFound):
                    await message.channel.send(":x: I couldn't download your skin file. Did you delete it?\n🛑 Skin render cancelled.")
                    for skinPath in skinPaths.values():
                        os.remove(skinPath)
                    bbGlobals.currentRenders.remove(itemObj.name)
                    return
                skinPaths[regionNum] = CWD + os.sep + bbConfig.paths.rendererTempFolder + os.sep + str(message.id) + "_" + str(regionNum) + ".jpg"
